 ...
 * after serves what is due. An occurrence that falls while the server is still
 * starting waits, on that same thread, for the full startup: the addons whose
 * services build the lines are not all up yet, and a line whose service is
...
public class DigestSenderJob {
  ...
  private static final String   STARTUP_SUBJECT = "Digest sender job";
    ...
    try {
      runner.execute(() -> {
        try {
          ...
          LOG.info("The digest run was interrupted while waiting for the full server startup: the server is stopping");
        } catch (Exception e) {
  ...
  /**
   * Returns once the server is fully started, HTTP connector included; returns
   * at once when it already is. Never called on the Spring scheduler thread.
   *
   * @throws InterruptedException when the server stops during the wait
   */
  protected void awaitServerStartup() throws InterruptedException {
    ServerStartupWaiter.awaitServerStartup(PortalContainer.getInstance(), STARTUP_SUBJECT);
  }
