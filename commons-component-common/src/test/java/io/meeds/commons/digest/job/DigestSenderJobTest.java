 ...
 * while a run is going, a failed run never blocks the next one, and a run
 * waits for the full server startup before it serves anyone.
 */
...
public class DigestSenderJobTest {
  ...
  public void setUp() throws Exception {
    // No server in a unit test: the wait returns at once
    doNothing().when(job).awaitServerStartup();
  }
  ...
  @Test
  public void testRunWaitsForTheFullServerStartupBeforeServing() throws Exception {
    job.run();
    verify(digestService, timeout(5000).times(1)).processDueDigests();

    InOrder inOrder = inOrder(job, digestService);
    inOrder.verify(job).awaitServerStartup();
    inOrder.verify(digestService).processDueDigests();
  }

  @Test
  public void testInterruptedWaitServesNobodyAndDoesNotBlockTheNextRun() throws Exception {
    doThrow(new InterruptedException("server stopping")).doNothing().when(job).awaitServerStartup();

    job.run();
    waitUntilIdle();
    verify(digestService, times(0)).processDueDigests();
